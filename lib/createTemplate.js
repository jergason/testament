 ...
 **/
function getTemplatedTestFile(pathToTestFiles, pathToTemplateFile, baseDir, requireModules) {
  return checkIfFileExists(pathToTestFiles)
    .then(getFileNames)
    .then(createTemplate(pathToTemplateFile, baseDir, requireModules))
}

function createTemplate(pathToTemplateFile, baseDir, requireModules) {
  return function(files) {
    return q.nfcall(fs.readFile, pathToTemplateFile, 'utf8').then(function(file) {
      var template = Handlebars.compile(file)

      files = files.map(function(f) {
        var name = addPrefix(baseDir, f)
      ...
      return template({files: files, requireModules: requireModules})
    })
  }
}
 ...
 */
function addPrefix(baseDir, filePath) {
