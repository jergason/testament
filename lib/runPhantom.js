...
function runPhantom() {
  ...
  var testFile = resolvedConfig.testHtml
  return function(PORT) {
    ...
    phantomExists.then(function(exists) {
      if (!exists) {
        ...
        // for some reason, this throws an error when using the .bin/mocha-phantomjs
        var mochaPhantomBinary = path.join(__dirname, '..', 'node_modules', 'mocha-phantomjs', 'bin', 'mocha-phantomjs')
